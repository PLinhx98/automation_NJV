...
user_api_key = keys.loc['api_key']['key']
...
def query(ro, params=None, user_api_key=user_api_key):
    ...
    while True:
        try:
            ...
            if ro.detail:
                if ro.query_id == 17067 or ro.query_id == '17067':
                    dir = r'G:/My Drive/Ninjavan Data/Fleet/Daily Task/pending_pu_details'
                    for f in os.listdir(dir):
                        os.remove(os.path.join(dir,f))
                    res.to_csv(f'{dir}/DATA_{end.day}_{end.month}_at_{end.hour}_{end.minute}.csv', index = False)
                    print('Overview EXPORTED!!!')
